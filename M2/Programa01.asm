...
inicializar_vetor: # Dinamicamente preenche o vetor ate a posicao informada
	...
	li t0, 0 # Inicializando variavel i(contador) com 0
...
inicializar_loop_vetor:
	...
	slli t1, t0, 2 # Calcula o offset: 4 * i
	add t2, s0, t1 # Calcula a posicao no vetor: comeco do vetor + (4 * i)
...
soma_vetor:
	...
	li t0, 0 # Inicializando variavel i(contador) com 0
	j loop_soma_vetor # Inicia o loop
	
loop_soma_vetor:
	bge t0, s1, soma_vetor_fim # Verificando se o numero de posições do vetor foi alcançado(se t0 >= s1 termina o loop)
	
	slli t1, t0, 2 # Calcula o offset: 4 * i
	add t2, s0, t1 # Calcula a posicao no vetor: comeco do vetor + (4 * i)
	lw t2, 0(t2) # Valor do vetor na posição i
	
	add s2, s2, t2 # Soma a posição atual do vetor com o valor de i (total += i)
	
	addi t0, t0, 1 # Incremento do contador: i = i + 1
	
	j loop_soma_vetor # "Reinicia o loop"  
...
main:
	...
	jal ra, inicializar_vetor # Chama a funcao inicializar_vetor e define ra para a proxima linha
	
	# Argumentos: 
	# a0 - Registrador de endereço base do vetor
	# a1 - Registrador do tamanho do vetor (numero de posições)
	jal ra, soma_vetor # Chama a funcao soma_vetor e define ra para a proxima linha
